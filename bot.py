    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            with open('import.log', 'rb') as log_file:
                await interaction.user.send("**Import successful** | Here is the log file for your information:", file=discord.File(log_file, 'import.log'))
...
async def send_progress(total, completed, progress_message):
    progress = (completed / total) * 100
    await progress_message.edit(content=f"**Progress:** {progress:.2f}% complete")
...
async def perform_import(api_url, api_key, interaction):
    ...
    progress_message = await interaction.followup.send("**Progress:** 0% complete", ephemeral=True)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for player in all_players:
            ...
            if completed_players % 10 == 0:
                await send_progress(total_players, completed_players, progress_message)
    ...
    await progress_message.edit(content="**Import complete!** ✅ You will receive a direct message with the log file.")
